...
interface OrderType { id: number; name: string; isActive: boolean; createdAt: string }
...
export default function OrderTypesPage() {
  ...
  const load = async () => {
    setIsLoading(true)
    try {
      const res = await apiClient.getOrderTypesList()
      if (res.success) setItems(Array.isArray(res.data) ? res.data : [])
    } catch { toast.error('Не удалось загрузить типы заказов') }
    finally { setIsLoading(false) }
  }

  useEffect(() => { load() }, [])

  const startEdit = (item: OrderType) => { setEditingId(item.id); setForm({ name: item.name, isActive: item.isActive }); setShowAdd(false) }
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      if (editingId) { await apiClient.updateOrderType(editingId, form); toast.success('Обновлено') }
      else { await apiClient.createOrderType(form); toast.success('Добавлено') }
  ...
  const remove = async (id: number) => {
    if (!confirm('Удалить тип заказа?')) return
    try { await apiClient.deleteOrderType(id); toast.success('Удалено'); setItems(items.filter(i => i.id !== id)) }
    ...
    <div className={ds.pageRoot}>
      <div className={ds.content}>
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className={ds.title}>Типы заказов</h1>
            <p className={ds.subtitle}>Классификация заказов для отчётов и фильтров.</p>
          </div>
        ...
        {showAdd && (
          <div className={`${ds.panelPadded} mb-6`}>
            ...
            <div className="flex gap-3 items-end flex-wrap">
              <input className={`${ds.input} max-w-md flex-1 min-w-[200px]`} placeholder="Название" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
