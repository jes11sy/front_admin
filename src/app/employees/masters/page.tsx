...
export default function MastersPage() {
  ...
  const { filteredAndSortedData, totalPages, paginatedData } = useMemo(() => {
    ...
    if (searchName.trim()) {
      ...
      filtered = filtered.filter(master => 
        ...
        master.login?.toLowerCase().includes(searchLower)
      )
    }
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDark ? 'bg-[#111113]' : 'bg-[#f5f5f7]'}`}>
      ...
      {/* Таблица */}
      {isLoading && <LoadingState isDark={isDark} message="Загрузка мастеров..." />}
      {!isLoading && loadError && <NetworkError isDark={isDark} onRetry={loadMasters} message={loadError} />}
          ...
          <tbody>
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((master) => (
                ...
                >
                  <td className={`py-3 px-4 ${isDark ? 'text-gray-300' : 'text-gray-800'}`}>{master.id}</td>
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation()
                          router.push(`/employees/masters/edit/${master.id}`)
                        }}
                        title="Редактировать"
                      >
